...
// 1
void rotation_x(point_t O, point_t* p, double angle_x)
{
	point_t temp;
...
void rotation_y(point_t O, point_t* p, double angle_y)
{
	point_t temp;
	memcpy(&temp, p, sizeof(point_t));
...
void rotation_z(point_t O, point_t* p, double angle_z)
{
	point_t temp;
	memcpy(&temp, p, sizeof(point_t));
...
// 3
void scaling_x(point_t O, point_t* p, double kx)
{
	p->x = (int)(O.x + kx * (p->x - O.x));
}

void scaling_y(point_t O, point_t* p, double ky)
{
	p->y = (int)(O.y + ky * (p->y - O.y));
}
void scaling_z(point_t O, point_t* p, double kz)
{
	p->z = (int)(O.z + kz * (p->z - O.z));
}

void scaling(figure_t *fig, change_t ch)
{
	for (int i = 0; i < fig->n_points; i++)
	{
